...
public class Scarabeus : ModNPC
{
	...
	public void Jumping(Player player)
	{
		...
		{ //jump towards player, at faster speed if farther away
			...
			hasjumped = true;
			NPC.noTileCollide = true;
			var JumpTo = new Vector2(player.Center.X, player.Center.Y - 300);
	...
	public void FlyDashes(Player player)
	{
		...
		{ //home in on player, but keeping some distance, also slowly rotating towards them
			...
			rotation = rotation.AngleLerp(ToPlayer.ToRotation(), 0.05f);
		}
		...
		else
		{
			...
			else if (NPC.velocity.Y <= 0) //check when the boss lands
			{
				...
				if (AiTimer % 7 == 0 && AiTimer < hometime + 61)
					for (int i = -1; i <= 1; i += 2)
					{
						var center = new Vector2(NPC.Center.X, NPC.Center.Y + NPC.height / 4);
	...
	public void Digging(Player player)
	{
		...
		if (InSolidTile)
		{
			...
			if (Main.netMode != NetmodeID.Server)
				for (int i = 0; i < 2; i++)
				{
					var dust = Dust.NewDustPerfect(NPC.Center + Vector2.UnitY * 20, Mod.Find<ModDust>("SandDust").Type, -Vector2.UnitY.RotatedByRandom(MathHelper.Pi / (12 + distfromsurface)) * distfromsurface * Main.rand.NextFloat(0.8f, 1.2f));
			...
			if (NPC.velocity.Length() > 4 && Main.netMode != NetmodeID.Server)
				for (int i = -2; i < 2; i++)
					if (i != 0)
					{
						var dust = Dust.NewDustPerfect(NPC.Center, Mod.Find<ModDust>("SandDust").Type, NPC.velocity.RotatedBy(Math.Sign(i) * MathHelper.Pi / 12));
			...
			{ //when the spin starts, save the initial velocity of the spin to rotate each tick, and store the player's center and a random spot far away from them
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
					for (int i = 0; i < numwaves; i++)
					{
						...
						var proj = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), spawnpos, Vector2.Normalize(statictarget[1] - spawnpos) * 6, ModContent.ProjectileType<SwarmTelegraph>(), 0, 0, Main.myPlayer);
	...
	private void SideDash(Player player, int hometime, int hometype = 0)
	{
		...
		{ //rotate backwards to telegraph dash
			...
			NPC.rotation = NPC.rotation.AngleLerp(NPC.spriteDirection * -MathHelper.Pi / 6, 0.1f);
			NPC.velocity = Vector2.Lerp(NPC.velocity, Vector2.Zero, 0.1f);
		}
		else if (AiTimer == hometime + 28)
	...
	private void SandSpit(Player player)
	{
		...
		else if (AiTimer > 0)
		{
			...
			if (AiTimer % 30 == 0)
				for (int i = -1; i <= 1; i += 2)
				{
					var center = new Vector2(statictarget[0].X, statictarget[0].Y + player.height / 4);
					center.X += 600 * i;
					int numtries = 0;
					int x = (int)(center.X / 16);
					int y = (int)(center.Y / 16);
