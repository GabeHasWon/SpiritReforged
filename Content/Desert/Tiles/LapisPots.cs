...
public class LapisPots : PotTile, ILootable
{
	...
	public override TileRecord AddRecord(int type, NamedStyles.StyleGroup group)
	{
		...
		return record.AddRating(2).AddDescription(Language.GetText(TileRecord.DescKey + ".Biome"));
	}
	...
	public override void AddItemRecipes(ModItem modItem, NamedStyles.StyleGroup group, Condition condition) => modItem.CreateRecipe()
		.AddRecipeGroup("ClayAndMud", 3).AddIngredient(AutoContent.ItemType<CarvedLapis>(), 2).AddTile(ModContent.TileType<PotteryWheel>()).AddCondition(condition).Register();
	...
	public override void SetStaticDefaults()
	{
		...
		DustType = IsRubble ? -1 : DustID.Cobalt;
	}
	...
	public override void KillMultiTile(int i, int j, int frameX, int frameY)
	{
		if (!IsRubble && Main.netMode != NetmodeID.MultiplayerClient && !WorldMethods.Generating)
		{
			...
			ItemMethods.SplitCoins((int)(CalculateCoinValue() * 1.5f), delegate (int type, int stack)
			{
	...
	public override void DeathEffects(int i, int j, int frameX, int frameY)
	{
		...
		for (int g = 1; g < 4; g++)
		{
			int goreType = Mod.Find<ModGore>("PotLapis" + g).Type;
	...
	{
		if (TileLootSystem.TryGetLootPool(ModContent.TileType<BiomePots>(), out var dele))
			dele.Invoke(loot);
	}
}