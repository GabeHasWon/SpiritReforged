...
public class SmokeCloud : DissipatingImage
{
	...
	public SmokeCloud(Vector2 position, Vector2 velocity, Color start, Color end, float scale, EaseFunction acceleration, int maxTime, bool useLightColor = true) : base(position, start, Main.rand.NextFloatDirection(), scale, 0.15f, "Smoke", new(0.33f, 0.33f), new(2, 1), maxTime)
	{
		UseLightColor = useLightColor;	
		startColor = start;
		endColor = end;
