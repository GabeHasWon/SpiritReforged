	...
	public sealed class RagePlayer : ModPlayer
	{
		...
		internal class RageHit : ModProjectile
		{
			...
			public override bool PreDraw(ref Color lightColor)
			{
				...
				float progress = EaseBuilder.EaseCircularIn.Ease(Progress / 0.5f);
				if (Progress > 0.5f)
					progress = EaseBuilder.EaseCircularOut.Ease(1f - (Progress - 0.5f) / 0.5f);

				Main.spriteBatch.Draw(starNonPreMult, Projectile.Center - Main.screenPosition, null, Color.DarkRed, 0f, starNonPreMult.Size() / 2f, 0.75f * progress, 0f, 0f);
