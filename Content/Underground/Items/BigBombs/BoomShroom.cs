...
public class BoomShroom : ModItem, IFlagged
{
...
internal class BoomShroomPlayer : ModPlayer
{
	...
	{
		if (Player.HasFlag<BoomShroom>() && SmallToLarge.ContainsKey(item.shoot))
			return 0.5f;
	...
	{
		if (Player.HasFlag<BoomShroom>() && SmallToLarge.TryGetValue(type, out int t))
			type = t;
	}
}
