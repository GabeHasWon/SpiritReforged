...
namespace SpiritReforged.Common.ItemCommon.MagazineSystem.UI.Dart;
public class DartUI
{
	public static readonly Asset<Texture2D> DartTexture = DrawHelpers.RequestLocal<DartUI>("DartUI", false);
	public static readonly Asset<Texture2D> DartOutline = DrawHelpers.RequestLocal<DartUI>("DartUI_Outline", false);

	public class DartUIAmmo(Vector2 position, Vector2 velocity, int timeLeft, bool empowered = false) : MagazineUIAmmo(position, velocity, timeLeft, empowered)
	{
		...
		{
			Texture2D texture = DartTexture.Value;
			Texture2D outlineTexture = DartOutline.Value;
	...
	public static MagazineUIAmmo OnEject(int empoweredCount)
	{
		...
		return new DartUIAmmo(position, Vector2.UnitX.RotatedByRandom(0.1f) * Main.rand.NextFloat(10f, 16f), 30, empoweredCount > 0);
	}
}
