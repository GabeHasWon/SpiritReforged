...
namespace SpiritReforged.Common.ItemCommon.MagazineSystem.UI.Bow;
public class BowUI
{
	public static readonly Asset<Texture2D> ArrowTexture = DrawHelpers.RequestLocal<BowUI>("BowUI", false);
	public static readonly Asset<Texture2D> ArrowOutline = DrawHelpers.RequestLocal<BowUI>("BowUI_Outline", false);

	public class BowUIAmmo(Vector2 position, Vector2 velocity, int timeLeft, bool empowered = false) : MagazineUIAmmo(position, velocity, timeLeft, empowered)
	{
		...
		{
			Texture2D texture = ArrowTexture.Value;
			Texture2D outlineTexture = ArrowOutline.Value;
	...
	public static MagazineUIAmmo OnEject(int empoweredCount)
	{
		...
		return new BowUIAmmo(position, Vector2.UnitX.RotatedByRandom(0.1f) * Main.rand.NextFloat(5f, 8f), 45, empoweredCount > 0);
	}
}
