...
public interface IDrawHeld
{
	...
	public static void DrawSwordHeld(ref PlayerDrawSet drawinfo, Texture2D texture, Rectangle? source = null, Color? color = null)
	{
		...
		color ??= Lighting.GetColor((int)drawinfo.drawPlayer.Center.X / 16, (int)drawinfo.drawPlayer.Center.Y / 16);

		drawinfo.DrawDataCache.Add(new DrawData(texture, drawPos, source, color.Value, rotation, source.Value.Size() / 2, 1, drawinfo.playerEffect, 0));
	}
}