...
public class OnFire : DoTExtension
{
    ...
    public override BuffSettings Settings => new(0.2f * VanillaScaling, (int)(500 * VanillaMaximum), false, Category.Fire);
    ...
    public override void PostDrawHealthBar(SpriteBatch spriteBatch, NPC npc, HealthBarHook.Options options)
    {
		...
		float activeProgress = Math.Min(_timeActive / 300f, 1);

		float fadeout = MathHelper.Min(BuffTime / 30f, 1);
        float lightness = 1f; //options.Lightness;
        float flameScale = options.Scale * Math.Min(fadeout, progress * 10) * 0.6f * (0.5f + activeProgress * 0.5f);
		...
		Color outlineColor = Color.Lerp(Color.Blue, Color.Red, activeProgress).Additive(150) * lightness;
