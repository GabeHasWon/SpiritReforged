...
float4 MainPS(VertexShaderOutput input) : COLOR0
{
    ...
    if (doDissolve)
        strength = smoothstep(min((dissolveNoiseStrength + dissolvePosBase + pow(dissolve, 6)) / 2, 1), 1, pow(strength, 0.34f)) * pow(strength, 0.66f);
    
    
    i.worldPos.x = floor(i.worldPos.x * 100) / 100;
    i.uv = floor(i.uv * 60) / 60;

    float2 perlinUv = float2(i.worldPos.x, _Time.y * 0.4);
    perlinUv.x -= pow(i.uv.y, 2) * _WaveCurve * _HorizontalScale;
    perlinUv.x *= _NoiseScale;
    float4 returnColor = tex2D(textureSampler, perlinUv) - i.uv.y;

    col += pow(invlerp(0, _LeadingEdgeDistance, i.uv.x), _LeadingEdgeexponent) * pow(invlerp(1, _TrailingEdgeDistance, i.uv.x), _TrailingEdgeExponent);
    col += invlerp(1, _TrailingEdgeDistance, i.uv.x) * invlerp(_TrailingEdgeDistance, _TrailingEdgeDistance + 0.4, i.uv.x) * 0.3;

    float originalAlpha = col.a;
    col.a = step(0.8, col);

    float cropTreshold = col.r;
    cropTreshold += _Time.z;
    cropTreshold = frac(cropTreshold * 1.8) * 1.4;
    float cropHeight = 0.65 * max(1 - i.uv.x * 2, 0);
    float noiseCrop = step(1.2, cropTreshold) * step(cropTreshold, 1.2 + cropHeight);
    col.a *= 1 - noiseCrop;
                
    col.rgb = min(col.rgb, 1.2);
    float innerMask = step(1, col.r + pow(i.uv.x, 3));
    col.rgb -= innerMask * float3(0.4, 0.35, 0.12);
    col.rgb += (1 - innerMask) * 2;
                
    col.rgb = floor(pow(col.rgb, 0.9) * 17) / 17;
